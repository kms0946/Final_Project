...
class PoseGraphic internal constructor(
  ...
  override fun draw(canvas: Canvas) {
    ...
    if(((180-Math.abs(angle24_26_28)) > 5) && !isCount){
      reInitParams()
      lineOneText = "똑바로 서주세요"
    }else if(yLeftHand>0 || yRightHand>0){
      reInitParams()
      lineOneText = "손을 머리 뒤로 잡아주세요"
    }else if(ratio<0.5 && isCount==false){
      reInitParams()
      lineOneText = "발을 어깨넓이로 벌려주세요."
    }else{
      ...
      if(!isDown && (currentHeight - lastHeight)>minSize){
        ...
        if((180-Math.abs(angle12_24_26))>90){
          lastHeight = currentHeight
          lineTwoText = "허리를 펴세요"
        }
        else if((180-Math.abs(angle24_26_28)) >100){
          lastHeight = currentHeight
          lineTwoText = "무릎을 굽히세요."
        }
